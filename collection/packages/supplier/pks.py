...
def _parse_detail_data(data=None, url=None, **kwargs):
    ...
    if lottery_type == 'HIGH_RATE':
        print('高频：', url)
        root_xpath = '//div[@data-type="{0}"]//tr'.format(url.split('-')[1])
        parse_xpath = {
            # 'root_xpath': '//div[@class="table h45 history-table"]//table//tr',  # table 列表
            'root_xpath': root_xpath,  # table 列表
            'date_xpath': '//div[@class="updata"]//text()',  # 高频开奖日期
            'latest_xpath': '//div[@class="cai-num pk10-num"]//text()',  # 最新一次开奖
            'latest_expect_xpath': '//div[@class="current"]//text()',  # 最新期号
            'remain_time_xpath': '//div[@class="djs"]//span//text()',  # 剩余开奖时间
            'expect_xpath': './/td[1]//text()',  # 期号 + 时间
            'time_xpath': './/td[2]//text()',  # 开奖时间
            'code_xpath': './/td[2]/div[1]//span//text()',  # 开奖结果
    ...
    for tr in tr_list:
        ...
        expect = int(util.number_format(expect_xpath[1]))
        hour_time = util.cleartext(expect_xpath[2])
        year_time = date_xpath[0].split('：')[-1] if date_xpath else use_date
        open_code = ','.join(code_list)
        cp_code = ''.join(code_list)
        open_time = year_time.split(' ')[0] if ':' in year_time else year_time
        open_time = open_time + ' ' + hour_time
        create_time = util.date()
        cp_id = cp_code  # 以中奖号+作为唯一id 并且开奖时间间隔大于15分钟  高频彩最低为20分钟，连着开同号概率极小
        cp_sn = int(str(kwargs.get('wid', )) + str(expect))
        # print('*' * 100)
        # print('expect', expect)
