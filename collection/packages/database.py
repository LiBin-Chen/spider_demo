...
def implode_field_value(array, glue=',', fields=None, encape='`'):
    ...
    添加exp，可用于直接执行SQL函数和语句如字段递增等; example: insert('order',data = {'num':('exp','num + 1')})
    '''
    args = []
    ...
    for k in array:
        if isinstance(array[k], (list, tuple)):
            ...
            elif array[k][0] in ('exp', 'EXP'):
                if glue == ',':
                    sql += "%s%s%s%s = %s" % (comma, encape, k, encape, array[k][1])
                else:
                    sql += "%s(%s%s%s %s)" % (comma, encape, k, encape, array[k][1])
            else:
                sql += "%s%s%s%s %s %%s" % (comma, encape, k, encape, array[k][0])
            ...
            if isinstance(arg, list):
                ...
                if len(arg) == 2:
                    ...
                    if opt in _comparison:
                        opt = _comparison[opt]
                    opt = opt.upper()
                    if opt in ('ILIKE', 'LIKE'):
                        query += "%s %s%s%s LIKE '%%%%%s%%%%'" % (comma, encape, arg[0], encape, arg[2])
                    elif opt in ('NOT LIKE0', '!LIKE'):
                        query += "%s %s%s%s NOT LIKE '%%%%%s%%%%'" % (comma, encape, arg[0], encape, arg[2])
                    else:
                        query += "%s %s%s%s %s %%s" % (comma, encape, arg[0], encape, arg[1])
        ...
        if table_name not in self.fields:
            ...
            if self.db_fields_cache:
                self.fields[table_name] = Util.file(
                    '.fields/%s/%s_%s_%s' % (self.host, self.__class__.__name__, self.dbname, table_name))
