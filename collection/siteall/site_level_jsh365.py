...
@description
    获取https://www.jsh365.com的彩票一级链接资料
...
def _parse_detail_data(data=None, url_key=None, url=None, **kwargs):
    ...
    if url_key == 1:
        prase_item = {
            ...
            'lottery_name': './/td[1]/a/text()',
            'expect': './/td[2]//text()',
            # 'lottery_newest_url': '//td//a//@href',  # 彩种最新开奖
            'open_time': './/td[3]//text()',
            'open_code': './/td[4]//text()',  # 一级不用
            'play_type': './/div[@class="abcdefg"]',  # 页面无玩法 abcdefg设置取空
            'lottery_detail_url': './/td[5]/a/@href',  # 彩种详情
            'lottery_history_url': './/td[6]/a/@href',
            'lottery_zs_url': './/td[7]/a/@href',
            'lottery_news_url': './/td[8]/a/@href',
            'interval_time': './/td[9]//text()',
        }
    ...
    for tr in tr_list:
        ...
        expect = str(int((util.number_format(expect[0]) if expect else '')))
        ...
        abbreviation = lottery_detail_url.split('-')[-1].split('.')[0]
        abb_pre = lottery_detail_url.split('-')[0].split('/')[-1]

        lottery_result = 'game_' + abb_pre + abbreviation + '_result' if url_key == 1 else 'game_' + abbreviation + '_result'
...
def save_data(href_name=None, href=None, time_list=None, data=None):
    ...
    '''
    if data:
        info = mysql.select('t_lottery_jsh', condition={'abbreviation': data['abbreviation']}, limit=1)
        if not info:
            mysql.insert('t_lottery_jsh', data=data)
            print('新增成功')
        else:
            mysql.update('t_lottery_jsh', condition={'id': info['id']}, data=data)
            print('修改成功')
    else:
        dlist = []
        ...
        for _item in dlist:
            # print('item', _item)
            info = mysql.select('t_lottery_jsh', condition={'abbreviation': _item['abbreviation']}, limit=1)
            if not info:
                mysql.insert('t_lottery_jsh', data=_item)
            else:
                mysql.update('t_lottery_jsh', condition={'id': info['id']}, data=_item)
...
if __name__ == '__main__':
    ...
    fetch_data(url_key=1, headers=_header, **kwargs)
