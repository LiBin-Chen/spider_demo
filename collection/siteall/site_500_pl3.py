...
def fetch_data(url, proxy=None, headers=None, **kwargs):
    ...
    try:
        ...
        # 将进行重试，可忽略
        _logger.info('STATUS:-400 ; INFO:数据请求异常 重新请求, %s ; URL:%s' % (util.traceback_info(e), url))
        try:
            if proxy:
                i = random.randint(0, proxy[0] - 1)
                proxies = {'http': 'http://' + proxy[1][i]}
...
def _parse_detail_data(data=None, url=None, **kwargs):
    ...
    for tr in tr_list[2:4]:
        ...
        # print(prize)
        # print(prize_number)
        if prize_name[0] in ['排列3直选', '排列三直选']:
            try:
                # print(prize)
                # print(prize_number)
        ...
        elif prize_name[0] in ['排列三组三', '排列3组三']:
            try:
                ...
                data_item['bonusSituationDtoList'][1]['numberOfWinners'] = 0
                data_item['bonusSituationDtoList'][1]['singleNoteBonus'] = 0
        elif prize_name[0] in ['排列3组六', '排列三组六']:
            try:
                ...
                data_item['bonusSituationDtoList'][2]['numberOfWinners'] = 0
                data_item['bonusSituationDtoList'][2]['singleNoteBonus'] = 0
        else:
            pass
...
def main(**kwargs):
    ...
    CP = {
        93: 'http://kaijiang.500.com/shtml/pls/19068.shtml',
    }
    RESULT = {
        93: 'game_pl3_result',
    }
    url = CP[93]
    dlt_list = get_expect('http://kaijiang.500.com/pls.shtml')
