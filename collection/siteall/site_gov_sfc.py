...
def save_data(url, db_name, item):
    ...
    if not info:
        ...
        _logger.info('INFO:  DB:%s 数据保存成功, 期号%s ; URL:%s' % (db_name, item['expect'], item['open_url']))
    else:
        mysql.update(db_name, condition=[('expect', '=', item['expect'])], data=item)
        _logger.info('INFO:  DB:%s 数据已存在 更新成功, 期号: %s ; URL:%s' % (db_name, item['expect'], item['open_url']))
...
def api_fetch_data(url=None, proxy=None, **kwargs):
    ...
    if cp_genre == '9':
        ...
        open_time = lottery['fTime'].split(' ')[0] + ' 10:00:00'
        # 3代表主队胜1代表主客队平0代表主负
    item = {
        'cp_id': cp_id,
        'cp_sn': cp_sn,
...
def main(**kwargs):
    ...
    FILE_DICT = {
        '4': r'C:\Users\admin\Desktop\lotto.txt',
        '5': r'C:\Users\admin\Desktop\pls.txt',
        '6': r'C:\Users\admin\Desktop\plw.txt',
        '8': r'C:\Users\admin\Desktop\qxc.txt',
        '9': r'C:\Users\admin\Desktop\sfc.txt',
    }
    ...
    while 1:
        ...
        if past:
            ...
            fetch_data(
                url='http://www.lottery.gov.cn/historykj/history.jspx?page=false&_ltype=dlt&termNum=0&startTerm=0&endTerm=19029',
                proxy=proxy, **kwargs)
...
def cmd():
    ...
    parser.add_argument('-p', '--past', help=u'默认下载500期历史数据',
